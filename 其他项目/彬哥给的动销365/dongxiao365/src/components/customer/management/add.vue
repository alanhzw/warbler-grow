<template>
  <div>
    ...
    <el-form :model="customerForm" :rules="customerRule" ref="customerForm" label-width="120px">
      <el-row>
        <el-col :span="18">
          <el-form-item label="客户名称：" prop="name">
            <el-input style="width:50%!important;" v-model="customerForm.name" placeholder="请输入客户名称" class="input-larger" :maxlength="25" :clearable=true></el-input>
          </el-form-item>
      ...
      <el-form-item label="联系人：" prop="contactList" required>
        <el-row  class="contact-list" v-for="(contact,index) in customerForm.contactList" :key="index">
            <el-col :span="6">
               <el-form-item prop="contactName" ref="contactName">
                <el-input v-model="contact.name" placeholder="请输入联系人姓名" class="input-large"  :clearable=true></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
               <el-form-item prop="contactTel" ref="contactTel">
      ...
      <el-form-item label="收货地址：" prop="addressList">
        <el-row v-for="(address,index) in customerForm.addressList" :key="index" class="contact-list">
          ...
          <el-col :span="6">
            <el-input v-model="address.tel" placeholder="请输入收货人电话" class="input-large" :clearable=true :maxlength="16"></el-input>
          </el-col>
          <el-col :span="8">
            <el-input v-model="address.address" placeholder="请输入收货地址" class="input-large" :maxlength="40" :clearable=true></el-input>
          </el-col>
          <el-col :span="4">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        customerRule: {
          name: [
            ...
            {max: 25, message: '长度在25个字符之内', trigger: 'change' }
          ],
          saleArea: [
            {required: true, validator: validateName1, trigger: 'change'}
          ],
          telephone: [
            {required: true, message: '请填写客户电话', trigger: 'change'}
          ],
          managerId: [
    ...
    methods: {
      ...
      verifyContact() {
        ...
        if (!contact.name || !contact.telephone) {
          ...
          this.$refs.contactName[0].$children[0].$refs.input.style.borderColor = '#f56c6c'
          this.$refs.contactTel[0].$children[0].$refs.input.style.borderColor = '#f56c6c'
        } else {
          this.showInputErr = false
          this.$refs.contactName[0].$children[0].$refs.input.style.borderColor = '#dcdfe6'
          this.$refs.contactTel[0].$children[0].$refs.input.style.borderColor = '#dcdfe6'
        }
      },
      // 新增,修改客户保存
      handleSubmit(formName) {
        const customerInfo = Object.assign({}, this.customerForm)
        const value = customerInfo.description
        customerInfo.description = this.transChara(value)
        if (customerInfo.address) { customerInfo.address = customerInfo.address.join(' '); }
        ...
        this.$refs[formName].validate((valid) => {
          if (valid && !this.showInputErr) {
            ...
            this.$http.post('/crm/customer/add', customerInfo, r => {
              this.$router.back();
    ...
    watch: {
      'customerForm.contactList': function(oldVal, newVal) {
        // console.log('watch', newVal[0].name, newVal[0].telephone)
