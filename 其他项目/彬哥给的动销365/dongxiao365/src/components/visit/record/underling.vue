<template>
  <div>
    ...
    <el-table
      border
      stripe
      v-loading="underlingData.loading"
      @sort-change="handleSort"
      :data="underlingData.data.list"
      ref="multipleTable"
      class="table-compact">
    ...
    <el-pagination
      ...
      v-if="underlingData.data.total"
      :total="underlingData.data.total">
    </el-pagination>
    <!--表单详情弹窗-->
...
<script>
  ...
  export default {
    ...
    methods: {
      loadData() {
        this.$store.dispatch('GET_UNDERLING_LIST', this.page)
      },
      // 加载组织架构数据
      loadDepartTree() {
