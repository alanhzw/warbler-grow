...
<script>
  ...
  export default {
    ...
    created() {
      this.selectAndInitFlow(this.$route.query);
      // 初始化图形渲染器
    ...
    methods: {
      selectAndInitFlow(id) {
        this.$http.get('/flow/info', id, r => {
      ...
      onSubmit() {
        ...
        this.$http.post('/flow/add', this.postData);
      }
    },
    computed: {
      listString() {
...
<style lang="scss">
  .jonit-container {
    ...
    overflow: auto;
    transition: none;
    background: white url("../../../assets/img/bb.png") repeat; //更改了路径
    .joint-element {
      cursor: pointer;
    }
    &.active {
