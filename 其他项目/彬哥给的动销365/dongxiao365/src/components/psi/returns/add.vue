    ...
    <el-form :inline="true" :model="order" ref="ruleForm" :rules="rules" label-width="110px">
      ...
      <el-row>
        ...
        <el-form-item label="业务员：" prop="salesmanName">
          <com-chouse-user-inline v-model="order.salesmanId"></com-chouse-user-inline>
        </el-form-item>
      </el-row>
      <hr>
        ...
        <el-form :model="dynamicValidateForm" ref="dynamicValidateForm" label-width="0px" class="table-dynamic-form">
          <el-table
            ...
            <el-table-column prop="returnPrice" label="退货价" v-if="order.returnType === 1">
              <template slot-scope="scope">
                <el-form-item
                  :prop="'commodityList.' + scope.$index + '.returnPrice'"
                  :rules="[{ required: true, message: '请输入退货价格', trigger: 'blur' }]">
                  <el-number v-model.number="scope.row.returnPrice" :min="1" align="center" class="input-full"></el-number>
                </el-form-item>
              </template>
            </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        order: {psiReturnDetailsRequestList: []},
        dynamicValidateForm: {
          commodityList: [
            {itemCount: 1}
          ]
    ...
    methods: {
      ...
      submit() {
        ...
        this.$http.post('/psi/return/add', this.order, r => {
          this.$message({
            type: "success",
            message: "保存成功!"
          });
