<template>
  <div class="ctrl-member-discount-editor">
    ...
    <form-block title="权益设置"
      ...
      <div slot="header">
        <lz-button size="mini"
                   v-show="data.discounts.length < 20"
                   type="primary"
                   @click="selectorShow = true">添加权益</lz-button>
        <lz-button size="mini"
                   @click="data.discounts = []">清空</lz-button>
      </div>
      <div v-if="data.discounts.length > 0">
        <div v-for="(discount, index) in data.discounts"
             class="discount-info discount-item"
             :key="`discount-${discount.id}`">
          <img class="product-pic"
               :src="`${CONST.IMAGE_PREFIX}${discount.showPics}`" />
          <div class="info-item">
            <p class="info-name">{{ discount.activityName }}</p>
            <p class="info-name">京东价：{{ discount.wareData && discount.wareData[0].showPrice }}</p>
            <p class="info-name">优惠价：{{ discount.wareData && discount.wareData[0].salePrice }}</p>

            <p>会员等级：{{ levels[discount.supportLevels] ? levels[discount.supportLevels].levelName : '' }}{{ levels.length > 0 && levels.length - 1 > discount.supportLevels ? '及以上' : '' }}</p>
          </div>
          <div class="float-icon">
            ...
            <div>
              ...
              <span class="iconfont icon-iconjiantou-34 btn-del"
                    :class="{ disable: index === data.discounts.length - 1 }"
                    @click="move(index, 1)" />
              <span class="iconfont icon-iconxx-31 btn-del"
                    :class="{ disable: index === data.discounts.length - 1 }"
    ...
    <lz-dialog title="会员专属权益"
      ...
      <div style="margin-bottom:20px"
        ...
        <lz-select v-model="shopId"
                   placeholder="请选择"
                   style="width:300px"
                   @change="getMemberdiscounts(1)">
          <lz-option v-for="item in shopList"
                     :key="item.shopId"
                     :label="item.shopName"
                     :value="item.shopId"></lz-option>
        </lz-select>
      ...
      <lz-table class="selector-table"
        ...
        <lz-table-column label="活动详情"
          ...
          <template slot-scope="scope">
            <div class="discount-info">
              <img class="product-pic"
                   :src="`${CONST.IMAGE_PREFIX}${scope.row.showPics}`" />
              <div class="info-item">

                <p class="info-name">{{ scope.row.activityName }}</p>
                <p class="info-name">京东价：{{ scope.row.wareData && scope.row.wareData[0].showPrice }}</p>
                <p class="info-name">优惠价：{{ scope.row.wareData && scope.row.wareData[0].salePrice }}</p>
        ...
        <lz-table-column label="活动状态"
                         align="center"
                         width="80">
          <template slot-scope="scope">{{ ['', '未开始', '进行中', '已结束', '已暂停'][scope.row.beginStatus] }}</template>
        </lz-table-column>
        <lz-table-column label="操作"
                         align="center"
                         width="80">
          <template slot-scope="scope">
            <lz-button v-if="data.discounts.some(c => c.id === scope.row.id)"
                       size="mini"
                       type="text"
      ...
      <div style="text-align: right;"
        ...
        <lz-pagination layout="total, prev, pager, next"
                       ...
                       @current-change="getMemberdiscounts"
...
export default {
  ...
  methods: {
    ...
    async getShopsList() {
      ...
      try {
        ...
        if (this.shopList.length > 0) {
          ...
          await this.getMemberdiscounts(1);
        }
      } catch (error) {
    ...
    async getMemberdiscounts(pageNo = this.page.pageNo) {
      ...
      try {
        await Service.getMemberCoupons({
          ...
          this.discountPool = data.filter(item => item.beginStatus != 3);
          this.page = {
            ...this.page,
            ...page,
          };
    ...
    add(discount) {
      if (this.data.discounts.length < 20) {
        this.data.discounts.push(discount);
      } else {
        LzMsg.error('权益数量不可超过20个');
      }
      this.$refs.formBlock.$refs.elForm.validateField('discounts');
    },
    move(index, offset) {
      if (index + offset < 0) {
        return;
      }
      const discount = this.data.discounts.splice(index, 1)[0];
      this.data.discounts.splice(index + offset, 0, discount);
    },
    remove(discount) {
      this.data.discounts = this.data.discounts.filter(c => c.id !== discount.id);
    },
    moveTop(index) {
      if (index === 0) {
        return;
