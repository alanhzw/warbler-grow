      ...
      <input-block title=""
        ...
        <lz-select size="small"
          ...
          <lz-option label="京豆"
                     :value="303"></lz-option>
          <lz-option label="礼品卡"
                     v-if='shopId!="703279" && !manlunList.includes(shopId)'
                     :value="304"></lz-option>
        </lz-select>
      </input-block>
    </form-block>
      ...
      <input-block title=""
        ...
        <div v-else>
          ...
          <div v-if="data.type==302">
            <div v-if="data.coupons.length > 0">
              <div v-for="(coupon, index) in data.coupons"
                ...
                <div class="info-item">
                  ...
                  <p>会员等级：{{levels[coupon.supportLevels-1] ? levels[coupon.supportLevels-1].levelName : ''}}{{levels.length > 0 && levels.length - 1 > coupon.supportLevels-1 ? '及以上' : ''}}</p>
                </div>
            ...
            <div v-else
                 class="blank-tip margin10">请添加积分兑换优惠券活动</div>
          </div>

          <div v-if="data.type==301">
            <div v-if="data.products.length > 0">
              <div v-for="(product, index) in data.products"
                ...
                <div class="info-item">
                  ...
                  <p>会员等级：{{levels[product.supportLevels] ? levels[product.supportLevels].levelName : ''}}{{levels.length > 0 && levels.length - 1 > product.supportLevels ? '及以上' : ''}}</p>
                </div>
                <div class="float-icon">
                  <div class="transform-180 ">
                    <span class="iconfont icon-iconxx-31 btn-del"
            ...
            <div v-else
                 class="blank-tip margin10">请添加积分兑换实物商品活动</div>
          </div>

          <div v-if="data.type==303">
            ...
            <div v-else
                 class="blank-tip margin10">请添加积分兑换京豆活动</div>
          </div>

          <div v-if="data.type==304">
            <div v-if="data.cards.length > 0">
              <div v-for="(product, index) in data.cards"
                   class="coupon-info coupon-item"
                   :key="`product-${product.id}`">
                <img class="product-pic"
                     :src="`${CONST.IMAGE_PREFIX}${product.showPics ? JSON.parse(product.showPics)[0] : ''}`" />
            ...
            <div v-else
                 class="blank-tip margin10">请添加积分兑换礼品卡活动</div>
          </div>
    ...
    <lz-dialog title="积分兑换"
      ...
      <lz-table class="selector-table"
        ...
        <lz-table-column label="活动详情"
          ...
          <template slot-scope="scope">
            <div class="coupon-info">
              <coupon-thumb v-if="data.type==302"
                            :info="scope.row" />
              <img class="product-pic"
                   v-if="data.type==301 || data.type==304"
        ...
        <lz-table-column label="操作"
                         ...
                         width="80">
          <template slot-scope="scope">
            <lz-button v-if="data.coupons.some(c => c.id === scope.row.id) || data.products.some(c => c.id === scope.row.id) || data.beans.some(c => c.id === scope.row.id)  || data.cards.some(c => c.id === scope.row.id)"
                       size="mini"
                       type="text"
...
export default {
  ...
  computed: {
    enough() {
      if (this.data.type === 301) {
        return this.data.products.length > 19;
      }
      if (this.data.type === 302) {
        return this.data.coupons.length > 19;
      }
      if (this.data.type === 303) {
        return this.data.beans.length > 19;
      }
      if (this.data.type === 304) {
        return this.data.cards.length > 19;
      }
      return false;
    },
  },
  ...
  watch: {
    ...
    'floorOnEdit.data.type': function () {
      ...
      this.data.cards = this.floorOnEdit.data.cards;
    },
  },
  methods: {
    // 获取店铺列表
    ...
    add(activity) {
      ...
      if (this.data.type === 304) {
        if (this.data.cards.length < 20) {
          this.data.cards.push(activity);
        } else {
          LzMsg.error('权益数量不可超过20个');
        }
      }
    ...
    remove(activity) {
      ...
      if (this.data.type === 304) {
        this.data.cards = this.data.cards.filter(c => c.id !== activity.id);
      }
    },
  },
};
