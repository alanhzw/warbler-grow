<template>
  <div class="ctrl-swipe-editor">
    <form-block title="轮播图片设置"
                ref="formBlock"
                :model="formData"
                :rules="rules">
      <input-block title="图片高度"
                   prop="height">
        <lz-input-number size="small"
                         v-model="data.height"
                         :min="120"
                         :max="500" />
        <p class="tip">注：图片推荐宽度750px，高度范围120px-500px，大小不超过1M图片类型为jpg、png</p>
      </input-block>
  ...
  computed: {
    ...
    rules() {
      const ruleObject = {
        height: {
          trigger: 'change',
          validator: (rule, value, cb) => this.validators.height(value),
        },
      };
      for (let i = 0; i < this.data.pics.length; i++) {
        ruleObject[`pic${i}`] = {
  ...
  methods: {
    imgValidator(width, height, size, type = '') {
      return new Promise((resolve, reject) => {
        ...
        } else if (['png', 'jpeg', 'jpg', 'image/jpeg', 'image/png', 'image/jpg'].indexOf(type) === -1) {
    ...
    selectImage(url) {
      ...
      this.$refs.formBlock.$refs.elForm.validateField(`pic${this.currentIndex}`);
