...
const BIZTYPE = 'jm-business-center';
...
const LZ_JD_CUSTOMER_TOKEN = 'LZ_JD_CUSTOMER_TOKEN';
...
const getToken = () => {
  return new Promise((resolve, reject) => {
    ...
    if (searchParams.token && searchParams.token !== '') {
      ...
      sessionStorage.setItem(LZ_JD_CUSTOMER_TOKEN, searchParams.token);
      ...
      const jdTokenInSession = sessionStorage.getItem(LZ_JD_CUSTOMER_TOKEN);
...
//       console.log(`${JD_WQ_AUTH_URL}?biz=${BIZTYPE}&url=${encodeURIComponent(authUrl)}`)
//       iframe.src = `${JD_WQ_AUTH_URL}?biz=${BIZTYPE}&url=${encodeURIComponent(authUrl)}`;
