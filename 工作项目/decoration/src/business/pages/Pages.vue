 ...
 * @LastEditTime: 2021-09-27 16:40:10
 * @Description:页面列表
-->
<template>
  <div class="page-pages">
    <lz-tabs v-model="type" class="page-tab"
             @tab-click="getPages">
      <lz-tab-pane label="会员中心"
                   name="0">
        <div class="bar">
          <label>访问地址：</label>
          <lz-input v-model="userCenter.url"
                    readonly></lz-input>
          <lz-tooltip class="item"
                      effect="dark"
                      content="复制链接"
                      placement="top">
            <img @click="copyText(userCenter.url)"
          ...
          <lz-tooltip class="item"
                      ...
                      content="获取短链接"
                      placement="top">
            <img @click="copyText(userCenter.shortUrl)"
          ...
          <!-- <lz-button class="table-button" text type="primary" @click="showData(nowLineIsReleased)">数据</lz-button> -->
          <div class="offset" />
          <!-- 现阶段最多允许创建20个页面 -->
          <lz-button class="btn-new"
                     v-if="userCenterPages.length < 20"
                     type="primary"
                     @click="openCreateDialog">创建新页面 (
            {{ userCenterPages.length }} / 20 )
          </lz-button>
          <lz-button class="btn-new"
                     v-else
                     type="primary"
                     disabled
                     size="small">创建新页面 ( {{ userCenterPages.length }} /
            20 )
          </lz-button>
        </div>
        <lz-table border
                  :data="userCenterPages">
          <lz-table-column label="名称">
            <template slot-scope="scope">
              <div class="cell-name">
                ...
                <img v-if="scope.row.onlineIsReleased === 1"
                     src="../../assets/icon-now.png"
                     style="margin-left:10px">
                <img v-if="scope.row.hasTask === 1"
            ...
            <template slot-scope="scope">
              ...
              <lz-operate-group-layout :max="4"
                ...
                <lz-button class="table-button"
                           text
                           type="primary"
                           :disabled="scope.row.onlineIsReleased === 1 || scope.row.hasTask === 1"
                           @click="remove(scope.row)">删除
                </lz-button>
                <lz-button class="table-button"
                           text
                           type="primary"
                           @click="showData(scope.row)">数据</lz-button>
              </lz-operate-group-layout>
            </template>
          </lz-table-column>
        </lz-table>
      </lz-tab-pane>
      <lz-tab-pane label="自定义页面"
                   name="1">
        <div class="bar">
          <!-- <label>访问地址：</label>
          <lz-input v-model="userCenter.url" readonly></lz-input>
          ...
          <lz-button class="btn-new"
                     ...
                     type="primary"
                     @click="openCreateDialog">创建新页面 (
            {{ customPages.length }} / 20 )
          </lz-button>
          <lz-button class="btn-new"
                     v-else
                     type="primary"
                     disabled>创建新页面 ( {{ customPages.length }} / 20 )</lz-button>
        </div>
        <lz-table border
                  :data="customPages">
          ...
          <lz-table-column label="操作"
            ...
            <template slot-scope="scope">
              ...
              qrcodeUrl: scope.row.onlineQrcodeUrl,
                ...
                <!-- <lz-button class="table-button" text type="primary" @click="showData(scope.row[0])">数据</lz-button> -->
              </lz-operate-group-layout>
            </template>
          </lz-table-column>
        </lz-table>
