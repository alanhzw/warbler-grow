<template>
  <div v-cloak>
    <div v-if="dayjs(new Date()).format('YYYY-MM-DD HH:mm:ss') > '2021-10-08 12:00:00'">
      <img id="kv" src="./assets/daMao922New.png"/>
    </div>
    <div v-else>
      <img id="kv1" src="./assets/daMaoKV.png"/>
    </div>
    ...
    <Overlay class="overlay" v-model:show="noQualifiedShow" :close-on-click-overlay="false">
      <div class="wrapper">
        <div class="failed-view">
          <div class="gotoShop" @click="gotoShopPage('1000147201')"></div>
          <div class="close-btn" @click="closeDialog('noQualified')"></div>
        </div>
      </div>
    </Overlay>
...
const openStatus = () => {
  // if (!isOpenCarded) {
  window.location.href = `https://shopmember.m.jd.com/shopcard?venderId=100000000000198&shopId=1000002672&venderType=1&channel=7038&returnUrl=${window.location.href}`;
  // }
};
// 领取记录接口
const prizeInfo = () => {
