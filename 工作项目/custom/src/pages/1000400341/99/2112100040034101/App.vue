<template>
  <div class="inxex" v-cloak>
    <div class="form">
      ...
      <div v-if="locationMask" class="box" style="margin: 0.55rem 0 0 1rem;">{{formData.location}}</div>
...
const getAddress = async (seqCode: number): Promise<any[]> => {
  const { data } = await cjwxRequest.post('/crm/kerastas/perfec/getAddress', {
    pin,
    activityId,
    seqCode,
  });
...
const gotoOpenCard = () => {
  ...
  window.location.href = `https://shopmember.m.jd.com/shopcard?venderId=1000400341&channel=401&returnUrl=${returnUrl}`;
};
...
const submit = () => {
  ...
  cjwxRequest.post('/crm/kerastas/perfec/save', formData.value)
    .then((data) => {
      if (data.result) {
        Toast('填写成功！');
        setTimeout(() => {
...
const activityContent = async () => {
  ...
  cjwxRequest.post('/crm/kerastas/perfec/activityContent', {
    pin,
    activityId,
  })
    .then(async (data) => {
