    ...
    if (res.result) {
      ...
      if (res.data.inviteDone) {
        ...
        if (res.data.helpStatus === 1) {
          ...
          // reportAssistEvent(baseInfo.shopId, baseInfo.pin, baseInfo.jdActivityId, myUuid.value);
          // reportClick(baseInfo.shopId, baseInfo.pin, baseInfo.jdActivityId, myUuid.value);
          reportClick('join', { shopId: baseInfo.shopId, pin: baseInfo.pin, jdActivityId: baseInfo.jdActivityId, shareUserId: myUuid.value, activityType: 40 });
          reportClick('assist', { shopId: baseInfo.shopId, pin: baseInfo.pin, jdActivityId: baseInfo.jdActivityId, shareUserId: myUuid.value, activityType: 40 });
      ...
      if (isOpenCarded.value && res.data.inviteDone === false) {
        setHeaderShare({
          ...
          shareUrl: `https://${window.location.host}/m/1000088787/99/yaoxin/?activityId=${activityId.value}&helpUuid=${myUuid.value}` as string,
          // eslint-disable-next-line no-use-before-define
...
const sharePoint = () => {
  ...
  reportClick('share', { shopId: baseInfo.shopId, pin: baseInfo.pin, jdActivityId: baseInfo.jdActivityId, shareUserId: myUuid.value, activityType: 40 });
};
// 分享好友
const share = () => {
  if (!isOpenCarded.value) {
...
const gotoCard = () => {
  ...
  // }
  openCardPoint();
  const returnUrl = encodeURIComponent(`https://${window.location.host}/m/1000088787/99/yaoxin/?activityId=${activityId.value}&helpUuid=${helpUuid}`);
