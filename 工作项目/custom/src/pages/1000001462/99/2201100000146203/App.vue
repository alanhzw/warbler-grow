...
<!--        />-->
      ...
      <div class="bottom-box">
        <img
          src="./assets/image/have-draw-btn.png"
          v-if="status === 1"
        />
        <img
          src="./assets/image/draw-btn.png"
          @click="convertPrize(status)"
          :class="{ 'filter-gra': status !== 2 }"
          v-else
        />
        <img
          src="./assets/image/prize-btn.png"
          @click="overlay.myGift = true"
        />
      </div>
      <div class="box box-wx">
        <div class="box-wx-text">
          每人限领取一次。获奖用户填写正确信息后，奖品以序列号方式发放，每个序列号仅可兑换一次，需在3个月内完成兑换。若因信息未填、填写错误、自行分享、超期未兑等用户原因导致奖品作废，不予补发。
        </div>
      </div>
      <div class="box">
        <img
  ...
  <van-popup v-model:show="overlay.openCard" :close-on-click-overlay="false">
    <div class="open-card">
      <img
        src="./assets/image/no-member.png"
        class="open-card-img"
      />
...
// 开卡链接
const goOpenCard = () => {
  const returnUrl = encodeURIComponent(`https://${window.location.host}/m/1000001462/99/2201100000146201/`);
