 ...
 * @FilePath:src\pages\726325\99\211272632501\hook\tool.ts
*/
...
export interface actInfoType {
  openFlg?:boolean, //  --> true-开卡；false-未开卡
  firstOrderDate?:boolean, //  -->  首购时间:true-满足；false-不满足
  giftNum?:number, //  -->  奖品剩余数量
  giftName?:string, //  -->  奖品名称
  rule?:string, //  -->  本期活动规则
  activityRule?:string, //  -->  活动规则
  activityFlag?:string, //  -->  活动期数标识  0非本期  1本期
  avardStatus?:string, // 获奖状态 0未获奖 1获奖 2待公布
  giftPicture?:string,
  limitPrice?:number,
  giftLeft?:boolean, // 是否还有库存  true 还有
  [propName:string] :any
}
...
export interface applyRecordType extends addressForm{
  giftRecordId?:string, //   -->  申领奖品（奖品名称）
  personalEmail?:string, //  -->  是否获奖（获奖状态：0 未获奖  1 获奖  2 待公布）
...
// 申领接口
export const subForm = async (form: addressForm, baseInfo:BaseInfo) => {
  let res;
  ...
  if (!form.receiver) {
    ...
    try {
      ...
      res = await cjwxRequest.post('diageo/newFirstPurchase/fillAddress', {
...
// 查询往期活动
export const findPreviousActInfo = async (baseInfo:BaseInfo) => {
  const actInfoList = ref([]);
  try {
    const res = await cjwxRequest.post('/diageo/newFirstPurchase/showGiftInfo', {
      activityId: baseInfo.activityId,
    });
...
export const getActInfo = async (baseInfo:BaseInfo) => {
  let res;
  try {
    res = await cjwxRequest.post('diageo/newFirstPurchase/activityContent', {
      activityId: baseInfo.activityId,
    });
    if (!res.result) {
      Toast.fail(res.errorMessage);
...
// 申请记录
export const getApplyRecord = async (baseInfo:BaseInfo) => {
  let res;
  try {
    Toast.loading({
      message: '加载中...',
    ...
    res = await cjwxRequest.post('diageo/newFirstPurchase/showApplyRecord', {
      activityId: baseInfo.activityId,
    });
    Toast.clear();
    if (!res.result) {
...
// 判断是否弹过窗
export const getIsShwoDialog = async (baseInfo:BaseInfo, giftId:string) => {
  let res;
  try {
