 ...
 * @FilePath:src\pages\726325\99\211272632502\App.vue
*/
<template>
  <img class='kv_img' src='./assets/kv.png'>
  <div class='log_dialog' @click='showPopUp.receivingRecord = true;'>我的申领记录 &gt;</div>
  <div class='log_dialog' @click='showPopUp.ruleDialog = true;' style='top: .73rem'>本期活动规则 &gt;</div>
  <!--  申领-->
  <div class='apply_gift'>
    <div class='apply' v-for='item in activityInfo.giftList' :key='item.skuId'>
      <img class='apply_img' :src='item.imageUrl'>
      <div class='apply_condition'>
        会员满{{item.money}}元享
      </div>
      <div class='apply_name'>{{item.giftName}}</div>
      <div class='apply_num'>限量{{item.memberCnt}}份</div>
      <div class='apply_btn'  :style='{filter:item.stock?"grayscale(0)":"grayscale(1)"}'  v-click-track='"点击参与人数"' @click='applySub(item)'>{{item.stock?"点击报名申领":"已领完"}}</div>
    </div>
  </div>
...
<!--      <div class='sku_tips line_clamp2'>{{ item.skuNameShort }}</div>-->
  ...
  <popup v-model:show='showPopUp.receivingRecord'>
    <div class='dialog'>
      <div class='log_con'>
        ...
        <div v-if='locationInfo.length>0'>
          <div class='log_list' v-for='item in locationInfo' :key='item.id'>
            <div class='log_til log_text'>{{ dayjs(item.createTime).format('YYYY-M-D')}}</div>
            <div class='log_til log_text'>{{ item.giftName }}</div>
            <div class='log_til log_text' @click='changelocation(item)'>{{item.rewardStatus ==="0"?'修改地址':'已填写'}}</div>
            <div class='log_til log_text'>{{ status2name(item.rewardStatus) }}</div>
          </div>
        </div>
  ...
  <popup v-model:show='showPopUp.ruleDialog'>
    <div class='ruleDialog'>
      ...
      <div class='dialog_rule' v-html='activityInfo.rule.replace(/\r\n/g, "<br/>")'></div>
  ...
  <popup v-model:show='showPopUp.showNoCondition '>
    <div class='modal'>
      <div class='modal_text'>很遗憾，您的购物金额 <br /> 未满足获奖条件，请继续加油。</div>
  ...
  <popup v-model:show='showPopUp.giftModal'>
    <div class='modal'>
      <div class='modal_text'>恭喜您获得满额礼申请资格,<br />请确认收货地址。</div>
      <div class='modal_hide' @click='changelocation(chooseLocationInfo.data,true) '>查看地址&gt;</div>
    </div>
  </popup>
  <!--地址-->
    ...
    <div class='location_modal'>
      <div class='form_box'>
        ...
        <div class='form_list'>
          ...
          <input class='form_input' maxlength='15' v-model.trim='form.name' placeholder='请输入姓名' type='text'>
        </div>
        <div class='form_list'>
          <div class='form_label'>手机号</div>
          <input class='form_input' maxlength='11' v-model.trim='form.phone' placeholder='请输入手机号' type='text'>
        </div>
        <div class='form_list'>
...
import {
  findSkus, addressForm, goMore,
  findPreviousActInfo, getRules, previousType, privacyClause,
  getActInfo, putInStatus, insertAddress, getLicationInfor, locationType, giftType, activityInfoType, skuListType, getIsShwoDialog,
} from './hook/tool';

Swiper.use([Autoplay]);
const baseInfo: BaseInfo = inject('baseInfo') as BaseInfo;
const form = ref<addressForm>({});
const skuList = ref<skuListType[]>([]);// 曝光商品
// 格式化中奖状态
const statusList = ['未公布', '中奖', '未中奖'];
...
const confirmAddress = (addressItemList: any) => {
  ...
  form.value.region = addressItemList[2].name;
...
const activityInfo = ref<activityInfoType>({});// 活动信息
const selectSku = ref<giftType>({}); // 申请的sku
const isShowDialog = ref(false);// 是否弹出过提示框
// 申请记录
const locationInfo = ref<locationType[]>([]);
// 要修改的地址信息
const chooseLocationInfo = reactive({
  data: {},
});
// 是否勾选协议
...
const saveForm = async () => {
  ...
  if (form.value.createTime) {
    // 修改地址
    const isSuccess = await insertAddress(form.value);
    if (isSuccess) {
      showPopUp.locationModal = false;
      Toast('修改成功~');
      // 重新查询申请记录
      locationInfo.value = await getLicationInfor(baseInfo);
    }
  } else {
    // 第一次添加地址申请
    const isSuccess = await insertAddress(form.value, baseInfo.activityId, selectSku.value, activityInfo.value, baseInfo.nickname, baseInfo.venderId);
    if (isSuccess) {
      showPopUp.locationModal = false;
  ...
  isAgreeInfo.value = false;
};
// 提交申请
const applySub = async (sku:any) => {
  if (!sku.stock) {
  ...
  selectSku.value = { ...sku };
  // 判断是否申请过
  const isReceived = await putInStatus(baseInfo, activityInfo.value.stage, sku.skuId);
  if (isReceived) {
    Toast('您已经申请过这个礼物了,请换一个礼物试试~');
    return;
  }
  showPopUp.locationModal = true;
};
// 修改地址
const changelocation = (data:addressForm, isNeedChangeLocation?:boolean) => {
  // 只有状态是未公布才能修改地址  当从中奖弹窗跳转过来时 可以修改
  if (data.rewardStatus !== '0' && !isNeedChangeLocation) return;
  isAgreeInfo.value = true; // 修改时默认勾选协议
  form.value = { ...data };
  addressCode.value = `${data.province}/${data.city}/${data.region}`;
...
// 处理申请记录数据，判断展示弹窗
const getShowModalType = async () => {
  let isHaveSuccess = false;// 有中奖的奖品
  let isHaveFailed = false;// 有未中奖的奖品
  locationInfo.value.forEach((item:locationType) => {
    // 弹窗展出判断 当期的申请数据  rewardStatus是成功或失败  当期没弹出过
    if (item.stage === activityInfo.value.stage) {
      if (item?.rewardStatus === '1' && !isShowDialog.value) {
        isHaveSuccess = true;
        chooseLocationInfo.data = item;
      }
      if (item?.rewardStatus === '2' && !isShowDialog.value) {
        chooseLocationInfo.data = item;
        isHaveFailed = true;
      }
    }
...
onMounted(async () => {
  ...
  [skuList.value, previousList.value, ruleText.value, activityInfo.value, locationInfo.value] = await Promise.all(
    [findSkus(baseInfo), findPreviousActInfo(baseInfo.activityId),
      getRules(baseInfo), getActInfo(), getLicationInfor(baseInfo)],
  );
  Toast.clear();
  isShowDialog.value = await getIsShwoDialog(baseInfo, activityInfo.value.stage); // 值为0未弹过,大于0弹过
  await getShowModalType();
  await nextTick(() => {
    initSwiper();
  });
});
