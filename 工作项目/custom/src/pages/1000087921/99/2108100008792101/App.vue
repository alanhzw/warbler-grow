...
<template>
  <div class="miqilin">
    ...
    <!-- 会员购买米其林耐越系列 -->
    <div class="product-view" style="top: 18.2rem">
      <!-- 耐越背景图 -->
      <div class="product-normal ny-bg"></div>

      <!-- 耐越按钮 -->
      <div class="product-btn-group">
        <div class="product-btn"
             :class="getBtnGray(list[2]?.prizeStatus)"
             @click="getBtnCheck(list[2])">{{ getBtnStatus(list[2]?.prizeStatus) }} >
        </div>
        <div class="product-btn"
             :class="getBtnGray(list[3]?.prizeStatus)"
             @click="getBtnCheck(list[3])" style="margin-left: .3rem">
          {{ getBtnStatus(list[3]?.prizeStatus) }} >
        </div>
      </div>
    </div>
