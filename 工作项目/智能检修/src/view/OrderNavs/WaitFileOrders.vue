 ...
 * @FilePath: \intelligent-overhaul-app\src\view\OrderNavs\WaitFileOrders.vue
-->
<template>
  <div class="header" v-if="user.userType != 2">
    <span>共{{ orderData.page.total }}个待归档工单</span>
  ...
  <van-popup v-model:show="isShow" position="bottom" :style="{ height: '95%' }">
    <popup-filter-leader v-if="isShow" @close="onClose" @changeData="getWaitFileOrders" />
  </van-popup>
</template>
...
const orderType = computed(() => [ null, 6, 5, 3 ][typeIndex])
const service = getService();

const getWaitFileOrders = (params) => {
