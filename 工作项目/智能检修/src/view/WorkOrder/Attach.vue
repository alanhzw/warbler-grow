 ...
 * @FilePath: \intelligent-overhaul-app\src\view\WorkOrder\Attach.vue
-->
<template>
  <!-- 班组长页面 -->
  <div class="attach" v-if='identity==="2"'>
    <common-title title='请上传挂牌照片' v-if="!isAttach"></common-title>
    <common-title title='无挂牌照片' v-if="isAttach && attachList.length===0"></common-title>
    <common-title title='挂牌照片' class="margin-bottom-32" v-if="isAttach && attachList.length>0"></common-title>
    <uploader ref='uploader' v-if="!isAttach"></uploader>
    <div v-if="isAttach && attachList.length > 0">
      <div class="attach-list-item" v-for='(item,index) in attachList' :key='index'>
        <div class="attach-name margin-bottom-32">挂牌人员：{{item.sysUserId}}</div>
        <div class="attach-list-divs">
          <div v-for='(pre,ur) in item.picList' :key='ur' class="attach-img-item">
  ...
  <div v-if='identity!=="2"' class="attach-list">
    <common-title title='等待班组长上传挂牌照片' v-if="!isAttach"></common-title>
    <common-title title='无挂牌照片' v-if="isAttach && attachList.length===0"></common-title>
    <common-title title='挂牌照片' class="margin-bottom-32" v-if="isAttach && attachList.length>0"></common-title>
    <div v-if="isAttach && attachList.length > 0">
      <div class="attach-list-item" v-for='(item,index) in attachList' :key='index'>
  ...
  setup (props) {
    ...
    const getAttach = () => {
      service.getAttach({
        ...
        // data.progressStatus = {
        //   createBy: "laibin",
        //   createTime: "2021-05-13 17:40:55",
        //   id: "1392776862245195777",
        //   showState: null,
        //   statusFive: "1",
        //   statusFiveDate: "2021-05-13",
        //   statusFour: "1",
        //   statusFourDate: "2021-05-13",
        //   statusOne: "",
        //   statusOneDate: "",
        //   statusSeven: "1",
        //   statusSevenDate: "2021-05-13",
        //   statusSix: "1",
        //   statusSixDate: "2021-05-13",
        //   statusThree: "1",
        //   statusThreeDate: "2021-05-13",
        //   statusTwo: "",
        //   statusTwoDate: "",
        //   sysOrgCode: "A01",
        //   trustFormMainTableId: "1386915583072493569",
        //   updateBy: "laibin",
        //   updateTime: "2021-05-26 00:02:27",
        // }
        state.status = data.progressStatus
        data.znjxBsnListingConfirm.forEach((item) => {
          item.picList.forEach((ele) => {
            if(ele.pic) {
              ele.pic = imgPath + ele.pic
            }
          })
        })
        state.attachList = data.znjxBsnListingConfirm
      })
    }
