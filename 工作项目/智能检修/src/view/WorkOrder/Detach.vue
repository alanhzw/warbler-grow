<!--
 * @Description:
 *      摘牌的页面
 * @Author:SXW
 * @Date: 2021-05-05
 * @LastEditTime: 2021-06-10
 * @FilePath: \intelligent-overhaul-app\src\view\WorkOrder\Detach.vue
-->
<template>
  <!-- 班组长页面 -->
  <div class="attach" v-if='identity==="2"'>
    <common-title title='请上传摘牌照片' v-if="!isAttach"></common-title>
    <common-title title='无摘牌照片' v-if="isAttach && attachList.length===0"></common-title>
    <common-title title='摘牌照片' v-if="isAttach && attachList.length>0"></common-title>
    <uploader ref='uploader' v-if="!isAttach"></uploader>
    <div v-if="isAttach && attachList.length > 0">
      <div class="attach-list-item" v-for='(item,index) in attachList' :key='index'>
        <div class="attach-name">摘牌人员：{{item.sysUserId}}</div>
        <div class="attach-list-divs">
          <div v-for='(pre,ur) in item.picList' :key='ur' class="attach-img-item">
            <img :src="pre.pic" @click="showImg(pre)">
          </div>
        </div>
      </div>
    </div>
    <bottom-button v-if='!isAttach'>
      <van-button type="info" color="#4FA0F7" @click='submit()'>提交</van-button>
    </bottom-button>
  </div>
  <!-- 作业长页面  组员页面 -->
  <div v-if='identity!=="2"' class="attach-list">
    <common-title title='等待班组长上传摘牌照片' v-if="!isAttach"></common-title>
    <common-title title='无摘牌照片' v-if="isAttach && attachList.length===0"></common-title>
    <common-title title='摘牌照片' v-if="isAttach && attachList.length>0"></common-title>
    <div v-if="isAttach && attachList.length > 0">
      <div class="attach-list-item" v-for='(item,index) in attachList' :key='index'>
        <div class="attach-name">摘牌人员：{{item.sysUserId}}</div>
        <div class="attach-list-divs">
          <div v-for='(pre,ur) in item.picList' :key='ur' class="attach-img-item">
            <img :src="pre.pic" @click="showImg(pre)">
          </div>
        </div>
      </div>
    </div>
  </div>
  <van-popup v-model:show="imgBoxShow">
      <img :src="previewImg" style="width: 6.8rem">
    </van-popup>
</template>
<script>
import BottomButton from '../../components/BottomButton.vue';
import CommonTitle from '../../components/CommonTitle.vue';
import Uploader from '../../components/Uploader.vue';
import { Dialog, Toast } from 'vant';
import { getService, imgPath } from '../../service';
import { useStore } from 'vuex';
import { useRouter, useRoute } from "vue-router";
import { ref, toRefs, reactive, computed, onMounted, watch } from 'vue';


// 工单摘牌页面
export default {
  components: { BottomButton, CommonTitle, Uploader },
  name: 'Attach',
  setup (props) {
    const uploader = ref(null);
    const service = getService();
    const store = useStore();
    const route = useRoute();
    const router = useRouter();
    const state = reactive({
      //当前身份 作业长1  班组长2  组员3
      identity: computed(() => store.state.user.info.userType),
      attachList: [],
      //各个阶段的状态
      status: {},
      //是否进行过摘牌
      isAttach: computed(() => {
        if (state.status.statusSeven && state.status.statusSevenDate) {
          return true
        } else {
          return false
        }
      }),
    });
    //提交
    const submit = () => {
      if (uploader.value.photoList.length === 0 && uploader.value.checked) {
        Dialog.confirm({
          title: '是否提交摘牌',
          message: '未上传任何摘牌照片',
          width: "5.4rem",
          confirmButtonColor: "#045BB8",
          confirmButtonText: "确定",
          className: "is-submit-attach",
        }).then(() => {
          //todos  确定的回调
          submitAttach()
        }).catch(() => {
          //todos  取消的回调
        })
      } else if (uploader.value.photoList.length === 0 && !uploader.value.checked) {
        Toast.fail({
          message: '请上传摘牌照片'
        })
      } else {
        submitAttach()
      }
    }
    //提交摘牌
    const submitAttach = () => {
      // if(!state.status.statusSixDate){
      //   Toast.fail({
      //     message: '验收通过审批才能摘牌'
      //   })
      //   return
      // }
      service.addTrustFormDelisting({
        pic: uploader.value.photoList.map((item) => {
          return item.url
        }).toString(),
        trustFormMainTableId: route.params.id
      }).then(res => {
        getAttach()
      })
    }
    //获取摘牌信息
    const getAttach = () => {
      service.getDelisting({
        trustFormMainTableId: route.params.id
      }).then((data) => {
        state.status = data.progressStatus
        data.znjxBsnDelistingConfirm.forEach((item) => {
          item.picList.forEach((ele) => {
            ele.pic = imgPath + ele.pic
          })
        })
        state.attachList = data.znjxBsnDelistingConfirm
      })
    }

    onMounted(() => {
      getAttach()
    })
    const imgBoxShow = ref(false)
    const previewImg = ref("")
    return {
      ...toRefs(state),
      submit,
      uploader,
      imgBoxShow,
      previewImg,
      showImg (pre) {
        imgBoxShow.value= true
        previewImg.value = pre.pic
      }
    }
  }
}
</script>
<style lang="scss">
.van-dialog {
  border-radius: 0.14rem;
  .van-dialog__message--has-title {
    padding-bottom: 0.32rem;
  }
}
.attach {
  padding: 0.32rem;
}
.is-submit-attach {
  .van-dialog__header {
    font-size: 0.36rem;
    line-height: 0.54rem;
    color: rgba(0, 0, 0);
  }

  .van-dialog__message--has-title {
    font-size: 0.3rem !important;
    color: #888888 !important;
  }

  .van-button__text {
    font-size: 0.36rem;
    line-height: 0.54rem;
  }
}
.attach-list {
  padding: 0.32rem;
}
.attach-list-item {
  display: flex;
  flex-direction: column;
  .attach-img-item {
  }
  .attach-name {
    text-align: left;
    font-size: 0.28rem;
    font-weight: 500;
    color: rgba(0, 0, 0, 0.8);
    line-height: 0.42rem;
  }
  .attach-list-divs {
    display: flex;
    justify-content: flex-start;
    flex-wrap: wrap;
    img {
      width: 80px;
      height: 80px;
    }
  }
}
</style>