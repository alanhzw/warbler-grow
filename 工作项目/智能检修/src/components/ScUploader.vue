 ...
 * @FilePath: \intelligent-overhaul-app\src\components\ScUploader.vue
-->
<template>
  <div class="uploader">
    <div class="SecurityClarification-top-uploader">
    ...
    <!-- <div class="SecurityClarification-bottom">
      <div class="SecurityClarification-bottom-title">无需/无法拍照</div>
      <div class="SecurityClarification-bottom-switch">
    ...
    changeChecked () {
      if (this.checked && this.photoList.length > 0) {
        this.$dialog.confirm({
          ...
          className: "is-delete-SecurityClarification-img",
        }).then(() => {
          this.checked = true
          this.photoList = []
...
<style lang='scss'>
.uploader {
  .SecurityClarification-top-uploader {
    width: 100vw;
    display: flex;
  ...
  .SecurityClarification-bottom {
    display: flex;
    justify-content: space-between;

    .SecurityClarification-bottom-title {
      font-weight: 500;
    ...
    .SecurityClarification-bottom-switch {
      margin-top: 0.22rem;
      .van-switch {
        width: 1rem;
        height: 0.6rem;
...
.is-delete-SecurityClarification-img {
  .van-dialog__header {
    font-size: 0.36rem;
    line-height: 0.54rem;
    color: rgba(0, 0, 0);
