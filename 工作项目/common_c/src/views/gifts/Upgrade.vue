...
export default {
  ...
  methods: {
    async getDetails() {
      try {
        const { data } = await PointsService.getUpgradeGiftList({});
    ...
    async getGifts() {
      try {
        const { data } = await PointsService.getUpgradeGiftResult({});
...
.bg {
    background: #000 url('./assets/up_bg.png') no-repeat;
    background-size: 100%;
