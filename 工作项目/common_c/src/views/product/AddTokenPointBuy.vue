<template>
  <!-- 积分加钱购 -->
  <div v-if="$store.state.showDetail" class="AddTokenPointBuy allpage">
    <i-swipe :swipers='details.showPicsParse'></i-swipe>
    <i-allDetial :details='details' :type='1'></i-allDetial>
    <i-condition :conditions='details.supportLevels'></i-condition>
    <i-rules :rules='details.activityRule' color='#C39441'></i-rules>
...
export default {
  ...
  methods: {
    // 获取详情
    async getDetail() {
      const res = await PointsService.getPointsPlusMoneySo({
    ...
    async exchangeConfirm() {
      ...
      // 替换接口
      try {
        const res = await PointsService.attendPointsPlusMoneySo(this.submitForm);
