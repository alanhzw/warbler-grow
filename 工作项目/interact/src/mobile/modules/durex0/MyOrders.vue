<template>
  <div class="durex-my-orders">
    <span class="btn"
          v-lz-click.isMain="'19'"
          @click="getMyPrizes"></span>
    <bottom-box title="您的订单"
                :visible.sync="ordersBoxShowed">
      ...
      <div v-else>
        <div class="order-item"
          ...
          <div style='width:47%;text-align: center;'>支付时间</div>
          <div style='width:28%;text-align: right;'>订单金额(元)</div>
        </div>
        <div class="order-item"
    ...
    async getMyPrizes() {
      try {
        ...
        const { data } = await ActivityService[this.settings.apis.getMyOrders]({
          ...
          activityType: this.activityInfo.activityType || 505,
          awardStartTime: '',
          priceLimit: '',
        });
